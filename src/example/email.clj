...
(def templates
  {:biff.auth/signup
   (fn [{:keys [biff.auth/link]}]
     {:subject "Sign up for example"
      :html (rum/render-static-markup
              [:div
               [:p "We received a request to sign up for example using this email address."]
   ...
   :biff.auth/signin
   (fn [{:keys [biff.auth/link]}]
     {:subject "Log in to example"
      :html (rum/render-static-markup
              [:div
               [:p "We received a request to log in to example using this email address."]
...
; :mailgun/from "example <contact@yourdomain.example.com>"
